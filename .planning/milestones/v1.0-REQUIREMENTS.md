# Requirements Archive: v1.0 SGFP4 v2 Decode (Vulkan-parity)
...
- [x] **SGV2-08**: LAYOUT_MIXED split-map parsing — 12-byte / 3-word little-endian bitmap, pre-order DFS traversal, quadrant order TL/TR/BL/BR, nodes of size ≥8 contribute a split bit, 4×4 nodes are always leaves
- [x] **SGV2-09**: Variable per-leaf decode for mixed records — leaf headers and payloads consumed in traversal order, each leaf's edge size n driving its payload word count, honoring 16-byte record and payload alignment
- [x] **SGV2-10**: Error-driven quadtree encoder — recursive subdivision with per-level MSE/relative-error thresholds, per-region mode selection (`choose T158 iff e_T158 ≤ (1+ε)·e_FP4`, default ε=0.10), ternary outlier veto, and uniform-layout collapse when all leaves share one size
- [x] **SGV2-11**: CPU round-trip tests for mixed/adaptive containers, including a golden split-map traversal-order check, passing via `./run_test.out`
...
| SGV2-07 | Minimal encoder + round-trip tests | Phase 1 | Complete |
| SGV2-08 | Quadtree split-map parse | Phase 2 | Complete (deferred formal verification — see MILESTONES.md) |
| SGV2-09 | Variable per-leaf decode | Phase 2 | Complete (deferred formal verification — see MILESTONES.md) |
| SGV2-10 | Error-driven encoder | Phase 2 | Complete (deferred formal verification — see MILESTONES.md) |
| SGV2-11 | Mixed round-trip tests | Phase 2 | Complete (deferred formal verification — see MILESTONES.md) |
