...
4. **Container adoption depth (P4): LOCKED 2026-08-22 — external-file + minimal descriptor, per Section 7's architecture research.** The SGFP4 v2 container (magic, record-offset table, macroblock records) is stored in a `.mnn.weight`-style external sidecar file; the op itself carries only a small `{magic, offset, size}` descriptor, mirroring `Convolution2D.external`'s existing `[offset, size]` pattern (already used by MNN's current quantized weights and reused generically by LLM export). A dedicated new Execution class (CPU + Vulkan) parses SGFP4's own offset-table/quadtree bytes internally — FlatBuffers schema never models macroblocks/quadtrees as typed fields.
...
2. ~~Container adoption depth...~~ **RESOLVED 2026-08-22: locked per Section 7** (external `.mnn.weight`-style file + minimal `{magic, offset, size}` descriptor in the op, mirroring `Convolution2D.external`; dedicated Execution class parses SGFP4 bytes internally). See Locked Decisions #4.
