...
| **Quick run command** | `cd .build; .\run_test.out op/sgfp4/classic_api` (filtered suite, PowerShell) |
| **Full suite command** | `cd .build; .\run_test.out op/sgfp4` (all SGFP4 suites; full `run_test.out` still blocked by pre-existing unrelated `FP4ModelTest.cpp` breakage — STATE.md) |
...
- **After every task commit:** Run `cd .build; .\run_test.out op/sgfp4/classic_api` (or `op/sgfp4` once later suites exist)
- **After every plan wave:** Run `cd .build; .\run_test.out op/sgfp4` + rebuild `sgfp4_inject.out` smoke (`--help` / demo-container E2E from Phase 5)
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 06-01-T1 (core refactor) | 06-01 | 1 | SGINJ-05 (build path) | T-06-01 | `sgfp4_inject_core.hpp` exists; `sgfp4_inject.cpp` main() thin; `sgfp4_inject.out` still builds and Phase 5 E2E unchanged | source assertion + build | `cd .build; cmake --build . --target sgfp4_inject.out; .\tools\fp4\sgfp4_inject.out --model minimal_512.mnn --niche-dir "<demo fp4 dir>" --output out.mnn; if (Test-Path out.mnn -and Test-Path out.mnn.weight) { "E2E-OK" } else { "E2E-FAIL" }` | no | pending |
...
> Incremental feedback (<30s) after first configure: rebuild `run_test.out` (filtered build) + run `cd .build; .\run_test.out op/sgfp4/classic_api` (PowerShell).
