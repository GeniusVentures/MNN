...
bool loadNicheDir(const std::string& dir, NicheDir& niche) {
    niche.dir = dir;
...
std::shared_ptr<MNN::OpT> makeDequantOp(const std::string& sidecarPath, size_t offset, size_t size, int dimO, int dimI) {
...
int injectMain(int argc, const char* argv[]) {
